...
if [ "$BUILD_CUML" == '1' ]; then
  ...
  if [ "$BUILD_ABI" == "1" ]; then
    ...
    conda build conda/recipes/dask-cuml -c conda-forge/label/cf201901 -c numba -c rapidsai/label/cf201901-cuda${CUDA_REL} -c nvidia/label/cf201901-cuda${CUDA_REL} -c pytorch -c defaults --python=${PYTHON}
  fi
fi
